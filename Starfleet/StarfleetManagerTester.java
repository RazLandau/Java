...
public class StarfleetManagerTester {
	...
	private static List<Spaceship> generateStarfleet() {
		...
		fleet.add(new ExplorationShip("USS Enterprise", 2451,7.2f, generateCrew (14,64), 9));
		fleet.add(new ExplorationShip("USS Andromeda", 2532,4.5f, generateCrew (15,31), 3));
		fleet.add(new ExplorationShip("USS Excalibur", 2532,4.5f, generateCrew (15,31), 6));
	...
	private static Set<CrewMember> generateCrew (int numberOfOfficers, int numberOfCrewmen) {
		...
		for (int i=0; i< numberOfCrewmen; i++) {
			...
			crew.add(new CrewWoman(name,age,yearsInService));
		}
